...
if __name__ == "__main__":
    ...
    for i in range(TRIALS):
        ...
        timeHelper.sleep(trajT1.duration * TIMESCALE + 2.0)
        
        
        cf0.uploadTrajectory(0, 0, trajU1)
        cf1.uploadTrajectory(0, 0, trajU2)
        cf2.uploadTrajectory(0, 0, trajU3)
        cf3.uploadTrajectory(0, 0, trajU4)
        cf4.uploadTrajectory(0, 0, trajU5)
        cf5.uploadTrajectory(0, 0, trajU6)
        cf0.goTo(pos_T[0,:], 0, 2.0)
        cf1.goTo(pos_T[1,:], 0, 2.0)
        cf2.goTo(pos_T[2,:], 0, 2.0)
        cf3.goTo(pos_T[3,:], 0, 2.0)
        cf4.goTo(pos_T[4,:], 0, 2.0)
        cf5.goTo(pos_T[5,:], 0, 2.0)

        timeHelper.sleep(2.5)
        allcfs.startTrajectory(0, timescale=TIMESCALE)
        timeHelper.sleep(trajU1.duration * TIMESCALE + 2.0)

        cf0.uploadTrajectory(0, 0, trajE1)
        cf1.uploadTrajectory(0, 0, trajE2)
        cf2.uploadTrajectory(0, 0, trajE3)
        cf3.uploadTrajectory(0, 0, trajE4)
        cf4.uploadTrajectory(0, 0, trajE5)
        cf5.uploadTrajectory(0, 0, trajE6)
        cf6.uploadTrajectory(0, 0, trajE7)
        cf0.goTo(pos_U[0,:], 0, 2.0)
        cf1.goTo(pos_U[1,:], 0, 2.0)
        cf2.goTo(pos_U[2,:], 0, 2.0)
        cf3.goTo(pos_U[3,:], 0, 2.0)
        cf4.goTo(pos_U[4,:], 0, 2.0)
        cf5.goTo(pos_U[5,:], 0, 2.0)
        cf6.goTo(initial_pos[6,:], 0, 2.0)
        timeHelper.sleep(2.5)
        allcfs.startTrajectory(0, timescale=TIMESCALE)
        timeHelper.sleep(trajE1.duration * TIMESCALE + 2.0)
