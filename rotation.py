...
if __name__ == "__main__":
    ...
    reference = distance[0]   #distance of the drone the is used as relative to

    rotation_factor = []
    for i in range(0,len(cf)):
        if distance[i] != reference:
            rotation_factor[i] = distance[i] / reference
        else:
            rotation_factor[i] = 1
    ...
    theta =[]
    theta[0] = 0
    for i in range (0,2*math.pi/0.05):
        if theta[i] != 2*math.pi:
            theta[i+1] = theta[i] + 0.05
    
    # theta = math.linsapce(0, 2*math.pi, 0.05) 
    # circle_position = [R*math.sin(theta), R*math.cos(theta)]

    for i in range(0,len(rotation_factor)):
        circle_position[i] = [R*math.sin(theta[i]), R*math.cos(theta[i])]
