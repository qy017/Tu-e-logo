...
if __name__ == "__main__":
   ...
   # cf6 = swarm.allcfs.crazyflies[6]
    ...
    traj_helix = uav_trajectory.Trajectory()
    traj_helix.loadcsv("helix1.csv")
    ...
    for i in range(TRIALS):
        ...
        for cf in allcfs.crazyflies:
            cf.uploadTrajectory(2, 0, traj_helix)
        ...
        allcfs.startTrajectory(2, timescale=TIMESCALE * 2, relative = True)
        timeHelper.sleep(traj_helix.duration * TIMESCALE + 1.0)

        allcfs.startTrajectory(2, timescale=TIMESCALE * 2, reverse = True, relative = True)
        timeHelper.sleep(traj_helix.duration * TIMESCALE + 1.0)
